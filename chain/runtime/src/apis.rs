//! Native and Wasm runtime APIs required by the pinned omni-node/tooling.
...
impl Runtime {
    ...
    fn can_build_upon(hash: <Block as BlockT>::Hash, slot: cumulus_primitives_aura::Slot) -> bool {
        ConsensusHook::can_build_upon(hash, slot)
    }
}

impl_runtime_apis! {
    impl sp_consensus_aura::AuraApi<Block, sp_consensus_aura::sr25519::AuthorityId> for Runtime {
        fn slot_duration() -> sp_consensus_aura::SlotDuration { Runtime::slot_duration() }
    ...
    impl cumulus_primitives_aura::AuraUnincludedSegmentApi<Block> for Runtime {
        fn can_build_upon(hash: <Block as BlockT>::Hash, slot: cumulus_primitives_aura::Slot) -> bool {
            Runtime::can_build_upon(hash, slot)
        }
    }
    ...
    impl frame_try_runtime::TryRuntime<Block> for Runtime {
        fn on_runtime_upgrade(checks: frame_try_runtime::UpgradeCheckSelect) -> (Weight, Weight) {
            let consumed = Executive::try_runtime_upgrade(checks)
                .expect("the fixed runtime upgrade dry-run must succeed");
            (consumed, crate::configs::RuntimeBlockWeights::get().max_block)
        }
        ...
        fn execute_block(
            ...
            Executive::try_execute_block(block, state_root_check, signature_check, select)
                .expect("the fixed runtime block dry-run must succeed")
        }
    }
    ...
    impl frame_benchmarking::Benchmark<Block> for Runtime {
        ...
        fn dispatch_benchmark(config: frame_benchmarking::BenchmarkConfig) -> Result<Vec<frame_benchmarking::BenchmarkBatch>, alloc::string::String> {
            ...
            if batches.is_empty() { Err("Benchmark not found for this pallet.".into()) } else { Ok(batches) }
        }
    }
    ...
    impl cumulus_primitives_core::GetParachainInfo<Block> for Runtime {
        fn parachain_id() -> cumulus_primitives_core::ParaId { ParachainInfo::parachain_id() }
    }
